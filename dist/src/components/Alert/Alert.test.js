...
describe('Button', () => {
    test('renders the Alert', () => {
        render(React.createElement(Alert, { message: "Hello world!" }));
    });
    test('close button should not be in the document if isClose is equal to false', () => {
        const { queryByTestId } = render(React.createElement(Alert, { message: "Hello world!", isClosable: false }));
        expect(queryByTestId('close-button')).toBeNull();
    });
    test('close button should  be in the document if isClose is equal to true', () => {
        const { queryByTestId } = render(React.createElement(Alert, { message: "Hello world!", isClosable: true }));
        expect(queryByTestId('close-button')).toBeInTheDocument();
    });
    test('alert will be removed after clicking onclose', () => {
        render(React.createElement(Alert, { message: "This is a success message", type: "success", timeout: 5000, isClosable: true }));
    ...
    test('should close the alert after the specified timeout duration', async () => {
        const timeout = 3000;
        const { queryByText } = render(React.createElement(Alert, { message: "Test Alert", timeout: timeout }));
