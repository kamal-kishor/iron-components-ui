...
describe('Stack', () => {
    test('should render a stack with default props', () => {
        const { container } = render(React.createElement(Stack, null));
    ...
    test('should render a stack with custom props', () => {
        const { container } = render(React.createElement(Stack, { align: "center", direction: "row", justify: "space-between", isInline: true, spacing: 10, wrap: true, style: { backgroundColor: 'red' } }));
        const stackElement = container.firstChild;
        expect(stackElement).toBeInTheDocument();
