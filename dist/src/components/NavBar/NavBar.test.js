...
describe('NavBar', () => {
    test('renders with default props', () => {
        const { getByTestId } = render(React.createElement(NavBar, null));
    ...
    test('renders with custom props', () => {
        const { getByTestId } = render(React.createElement(NavBar, { width: "100%", bg: "blue", color: "white", style: { padding: '10px' } }));
        const navbarComponent = getByTestId('navbarComponent');
        expect(navbarComponent).toBeInTheDocument();
        expect(navbarComponent).toHaveStyle({
            width: '100%',
