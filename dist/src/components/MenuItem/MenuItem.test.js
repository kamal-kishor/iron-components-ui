...
describe('MenuItem', () => {
    test('should render with default props and invoke onClick when clicked', () => {
        ...
        const { getByText } = render(React.createElement(MenuItem, { onClick: onClickMock }));
    ...
    test('should render with custom props and apply appropriate classes', () => {
        const onClickMock = jest.fn();
        const { getByText } = render(React.createElement(MenuItem, { autofocus: true, children: "Custom Item", className: "customClass", disableGutters: true, onClick: onClickMock, value: "Custom Value" }));
